    ...
    function performHttpRequest(instance, jsonData, callback){
        ...
        var parseJson = function(res, data){
            ...
            if (res.statusCode === 500) {
                ...
                if (data.includes('Work queue depth exceeded')) {
                    // sleep for 10s such that the http work queue can recover
                    // There's no sleep function in nodejs so a blocking
                    // ad hoc implementation is done here.
                    const crypto = require('crypto')
                    const sleep_until = Date.now() + 10000
                    while (Date.now() < sleep_until) {
                        crypto.pbkdf2Sync('busy', 'sleep', 1e6, 64, 'sha256')
                    }
                }
                return
            }
